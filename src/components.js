// Dipendenze da React e utils.js (assumendo che siano caricate globalmente)
// const { useState, useEffect, useRef, useMemo } = React; // Già definito in utils.js
// const { Icons, vibrate } = window; // Assumendo che siano globali
    ...
    return (
        <div className="bg-white p-4 rounded-2xl shadow-sm mb-4 border border-gray-100">
            ...
            <div className="mt-3 text-xs font-bold text-center">
                {remaining > 0 ? (
                    <span className="text-red-500">Mancano <span className="text-sm">€ {remaining.toFixed(2)}</span> per lo sconto!</span>
                ) : (
